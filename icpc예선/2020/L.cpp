...
void convolution(std::vector<std::complex<double>> &a, std::vector<std::complex<double>> &b) {
    /*
        ...
        Let k such that max(N`: N` < 2 * N with N` = 2^k) and a`, b` below.
    ...
    */
    ...
    unsigned long long n = a.size() + b.size();
    unsigned np = n, i;
...
int main() {
    int a, b, c;
    std::cin >> a;
    std::vector<std::complex<double>> A(60001,0), C(60001, 0);
    for(int i = 0 ; i < a ; i++) {
        int x;
        std::cin >> x;
        A[x + 30000] = 1;
    ...
    for(int i = 0 ; i < 60001 ; i++) {
        answer += B[i] * (int) std::round(A[2 * i].real());
    }
    std::cout << answer << "\n";
}